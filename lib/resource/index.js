...
module.exports = MongoResource = new Class({
	...
	, create_object: function create_object( bundle, callback ){
		var obj = new this.options.objectTpl();
		merge(obj, bundle.data);
	...
	, replace_object: function replace_object( bundle, callback ){
		...
		this.full_hydrate( bundle, function( hyd_err, bundle ){
			...
			Model.update( {_id: pk}, bundle.object ).exec(function( err, response){
				callback( err, bundle );
			});
		});
		return this;
	...
	, sort: function sort( mquery, rquery ){
		var ordering = {};
		var allowed = this.options.ordering || EMPTY_ARRAY;
	    var params = toArray( rquery.orderby );
		var bits, x, len;
